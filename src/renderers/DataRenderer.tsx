    ...
    <section className="code-renderer" data-testid="code-renderer">
      <div className="renderer-toolbar">
        <span className="file-name">
          <Code2 /> {envelope.payload.fileName ?? "untitled"}
          <b>{envelope.payload.language}</b>
        </span>
        ...
        {envelope.payload.editable ? (
          <button
            className="icon-button"
            type="button"
            onClick={() => setEditing((value) => !value)}
          >
            <Code2 /> {editing ? "Preview" : "Edit"}
          </button>
        ) : null}
      </div>
      {large ? (
        <div className="guard-banner">
          Large file guard: showing the first 2,000 of{" "}
      ...
      {collapsed ? null : editing ? (
        ...
        <pre className={wrap ? "code-lines wrap" : "code-lines"}>
          {visibleLines.map((line, index) => {
            const match =
              query && line.toLowerCase().includes(query.toLowerCase());
            const diffClass = envelope.payload.diff
              ? line.startsWith("+")
                ? "diff-add"
                : line.startsWith("-")
                  ? "diff-remove"
            ...
            return (
              <span
                className={`${match ? "search-hit" : ""} ${diffClass}`}
                key={`${index}-${line.slice(0, 12)}`}
              >
                <i>{index + envelope.payload.startLine}</i>
                <code>{line || " "}</code>
              </span>
            );
          })}
...
function MathView({ envelope }: { envelope: MathEnvelope }) {
  ...
  let html = "";
  try {
    html = katex.renderToString(envelope.payload.latex, {
      displayMode: envelope.payload.display === "block",
      throwOnError: true,
      strict: "warn",
      trust: false,
    });
  ...
  return error ? (
    <div className="renderer-state" role="alert">
      <h2>Formula error</h2>
      <p>{error}</p>
      <pre>{envelope.payload.latex}</pre>
    </div>
  ) : (
    <div
      data-testid="math-renderer"
      className={`math-renderer ${envelope.payload.display}`}
...
function JsonNode({
  ...
  if (value === null || typeof value !== "object") {
    ...
    return (
      <span
        className={
          query && stringValue.toLowerCase().includes(query.toLowerCase())
            ? "search-hit"
            : `json-${typeof value}`
        }
      >
  ...
  return (
    <span className="json-node">
      ...
      <span>{brace[0]}</span>
      {!open ? (
        <span className="json-count"> {entries.length} items </span>
      ) : null}
      {open ? (
        <span className="json-children">
          {entries.map(([key, child], index) => {
            const childPath = Array.isArray(value)
              ...
              : `${path}.${key}`;
            return (
              <span className="json-entry" key={childPath}>
                <button
                  className="json-key"
                  type="button"
                  title={`Copy ${childPath}`}
                  onClick={() => void copy(childPath)}
                >
...
function JsonView({ envelope }: { envelope: JsonEnvelope }) {
  ...
  const serialized = useMemo(
    () => JSON.stringify(envelope.payload.data, null, 2),
    [envelope],
  );
  return (
    <section className="json-renderer" data-testid="json-renderer">
      <div className="renderer-toolbar">
        <span className="file-name">
          <FileJson /> {envelope.payload.format.toUpperCase()}
        </span>
        ...
        <label className="toolbar-search">
          <Search />
          <span className="sr-only">Search structured data</span>
          <input
            value={query}
            onChange={(event) => setQuery(event.target.value)}
      ...
      {serialized.length > 1_000_000 ? (
        <div className="guard-banner">
          Large data guard active. Tree depth is limited.
        </div>
      ) : null}
      {mode === "raw" ? (
        <pre className="json-raw">{serialized}</pre>
      ) : (
        <div className="json-tree">
          <JsonNode value={envelope.payload.data} path="$" query={query} />
        </div>
      )}
    </section>
  );
...
function TableView({
  ...
  const rows = useMemo(() => {
    let result = envelope.payload.rows.map((row, index) => ({
      row,
      originalIndex: index,
    }));
    if (query)
    ...
    if (sort)
      result.sort(
        (left, right) =>
          String(left.row[sort.key] ?? "").localeCompare(
            String(right.row[sort.key] ?? ""),
            undefined,
            { numeric: true },
          ) * sort.direction,
      );
  ...
  const pageCount = Math.max(
    1,
    Math.ceil(rows.length / envelope.payload.pageSize),
  );
  const visibleRows = rows.slice(
    page * envelope.payload.pageSize,
    (page + 1) * envelope.payload.pageSize,
  );
  const visibleColumns = envelope.payload.columns.filter(
    (column) => !hidden.has(column.key),
  );
  const selectedRows = envelope.payload.rows.filter((_row, index) =>
    selected.has(index),
  );
  const numericValues = selectedRows.flatMap((row) =>
  ...
  const exportCsv = () => {
    const csv = [
      visibleColumns.map((column) => escapeCsvCell(column.label)).join(","),
      ...rows.map(({ row }) =>
        visibleColumns
    ...
    anchor.href = url;
    anchor.download = `${envelope.id}.csv`;
    anchor.click();
    ...
    <section className="table-renderer" data-testid="table-renderer">
      <div className="renderer-toolbar table-tools">
        ...
        <details className="column-picker">
          ...
          <div>
            {envelope.payload.columns.map((column) => (
        ...
        <button
          ...
          onClick={onChart}
        >
          <BarChart3 /> Convert to chart
        </button>
      </div>
      <div className="table-scroll">
        <table>
          <caption>{envelope.payload.caption ?? `${rows.length} rows`}</caption>
          <thead>
            <tr>
          ...
          <tbody>
            {visibleRows.map(({ row, originalIndex }) => (
              <tr
                key={originalIndex}
                className={selected.has(originalIndex) ? "selected" : ""}
              >
      ...
      {selected.size ? (
        <div className="selection-summary">
          ...
          <span>Numeric cells: {numericValues.length}</span>
          <span>
            Sum:{" "}
            {numericValues
...
function NotebookView({ envelope }: { envelope: NotebookEnvelope }) {
  ...
  if (raw)
    return (
      <section className="notebook-renderer">
        <div className="renderer-toolbar">
          <button
            className="icon-button"
            type="button"
    ...
    <section className="notebook-renderer" data-testid="notebook-renderer">
      <div className="renderer-toolbar">
        <span className="toolbar-note">
          {envelope.payload.cells.length} cells · safe MIME output
        </span>
        <button
          className="icon-button"
          type="button"
      ...
      <div className="notebook-cells">
        {envelope.payload.cells.map((cell) => {
          const isCollapsed = collapsed.has(cell.id);
          return (
            <article className={`notebook-cell ${cell.cellType}`} key={cell.id}>
              <button
                className="cell-gutter"
                type="button"
                aria-label={`${isCollapsed ? "Expand" : "Collapse"} cell`}
                onClick={() =>
                  ...
                  {cell.cellType === "markdown" ? (
                    ...
                    <>
                      ...
                      {cell.outputs.map((output, index) => (
                        <div
                          className={`cell-output ${output.outputType}`}
                          key={index}
                        >
                          {output.text ? (
                            <pre>{output.text}</pre>
...
function ParquetView({ envelope }: { envelope: ParquetEnvelope }) {
  return (
    <section className="data-summary" data-testid="parquet-renderer">
      <p className="eyebrow">Columnar dataset</p>
      <div className="metric-grid">
        <div>
          <span>Rows</span>
          <strong>
            {envelope.payload.rowCount?.toLocaleString() ?? "Unknown"}
          </strong>
        </div>
        <div>
          <span>Columns</span>
          <strong>{envelope.payload.columns.length}</strong>
        </div>
        <div>
          <span>Preview</span>
          <strong>{envelope.payload.preview.length}</strong>
        </div>
      </div>
      <h3>Schema</h3>
      ...
      {envelope.payload.preview.length ? (
        <pre>{JSON.stringify(envelope.payload.preview, null, 2)}</pre>
      ) : (
        <div className="empty-state">
          Load a Parquet or Arrow fixture to inspect rows.
        </div>
      )}
    </section>
  );
...
export default function DataRenderer({
  ...
  if (envelope.type === "text.code")
    return <CodeView envelope={envelope as CodeEnvelope} />;
  if (envelope.type === "text.math")
    return <MathView envelope={envelope as MathEnvelope} />;
  if (envelope.type === "data.json")
    return <JsonView envelope={envelope as JsonEnvelope} />;
  if (envelope.type === "data.table")
    return (
      <TableView
        envelope={envelope as TableEnvelope}
        onChart={() =>
          onEvent?.({
  ...
  if (envelope.type === "data.notebook")
    return <NotebookView envelope={envelope as NotebookEnvelope} />;
  if (envelope.type === "data.parquet")
    return <ParquetView envelope={envelope as ParquetEnvelope} />;
