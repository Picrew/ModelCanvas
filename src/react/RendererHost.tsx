...
export interface RendererInspection {
  ...
  capabilities: ReturnType<
    typeof rendererRegistry.capabilities
  >[number]["supports"];
      ...
      .catch((error: unknown) => {
        ...
        const message =
          error instanceof Error
            ? error.message
            : "Renderer module failed to load";
  ...
  if (loadFailure?.id === resolution.definition.id) {
    return (
      <div className="renderer-state" role="alert">
        <p className="eyebrow">Module load failed</p>
        <h2>{loadFailure.message}</h2>
      </div>
    );
  }
    ...
    >
      <div className="renderer-root" data-renderer={resolution.definition.id}>
        <div
          ...
          title="Payload validated before renderer load"
        >
          <ShieldCheck aria-hidden="true" /> Validated ·{" "}
          {envelope.security?.trusted ? "trusted source" : "untrusted input"}
        </div>
