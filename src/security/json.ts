...
export function assertJsonWithinLimits(
  ...
  while (stack.length) {
    ...
    if (nodes > maxNodes) {
      return {
        ok: false,
        path: current.path,
        message: `Payload exceeds ${maxNodes} nodes`,
        code: "too_many_nodes",
      };
    }
    if (current.depth > maxDepth) {
      return {
        ok: false,
        path: current.path,
        message: `Payload exceeds depth ${maxDepth}`,
        code: "too_deep",
      };
    }
    ...
    if (seen.has(current.value)) {
      return {
        ok: false,
        path: current.path,
        message: "Payload contains a circular reference",
        code: "circular",
      };
    }
  ...
  return {
    ok: true,
    path: "$",
    message: "Payload is within limits",
    code: "ok",
  };
}
